...
function renderStepBody(langId, lesson, step) {
  ...
  if (step.type === 'summary') {
    return `
      ${lesson.common_mistake ? `<div class="panel lesson-block" style="border-color:color-mix(in srgb, var(--danger) 30%, transparent);"><h4 style="color:var(--danger);">⚠ Common Mistake</h4><div class="explanation">${lesson.common_mistake}</div></div>` : ''}
      <div class="panel why-box" style="margin-top:14px;">${lesson.why_it_matters}</div>
      ${renderAiPanel(langId, lesson)}
    `;
  }
...
function openAiSetup() {
  ...
  function renderFields(provider) {
    ...
    fields.innerHTML = `
      ...
      <select id="modelSelect" style="width:100%; background:color-mix(in srgb, var(--text) 5%, transparent); color:var(--text); border:1px solid var(--border); border-radius:12px; padding:12px 14px; font-family:'DM Sans',sans-serif; font-size:13px; margin:10px 0;">
