...
let state = { view: 'home', langId: null, lessonIdx: 0, history: [] };
...
function openLesson(langId, idx) { navigate('lesson', { langId, lessonIdx: idx }); }
...
async function runCode(langId, lesson) {
  ...
  if (langId === 'html') {
    wrap.innerHTML = `<iframe class="preview-frame" sandbox="allow-scripts"></iframe>`;
    ...
    wrap.innerHTML = `<iframe class="preview-frame" sandbox="allow-scripts"></iframe><div class="console-out" id="consoleOut"></div>`;
    const runner = `
...
function renderMixLesson(langId, lesson) {
  ...
  return `
    ...
    <div class="panel lesson-block">
      ...
      <div class="preview-wrap"><iframe class="preview-frame" id="mixFrame" sandbox="allow-scripts"></iframe></div>
    </div>
...
async function askAi(langId, lessonNum) {
  ...
  const provider = store.get('sc_provider', null);
  const key = provider ? store.get(`sc_key_${provider}`, null) : null;
  const model = provider ? store.get(`sc_model_${provider}`, null) : null;
  if (!provider || !key || !model) { openAiSetup(langId, lessonNum); return; }
  ...
  try {
    ...
    resultBox.innerHTML = `<div class="ai-response">${answer.replace(/\n/g,'<br>')}</div>`;
  } catch (err) {
    resultBox.innerHTML = `<div class="ai-response" style="color:var(--danger)">Error: ${err.message}</div>
