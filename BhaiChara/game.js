...
// ── Game State ─────────────────────────────────────────
let _db               = null;
let _getCurrentUser   = null;  // ✅ FIX: getter function — stale reference nahi hogi
let _gameListener     = null;
let _activeGameId     = null;
let _activeGameRef    = null;

// ── Helpers: always fresh values ───────────────────────
function getU()  { return _getCurrentUser ? _getCurrentUser() : null; }
function getCC() { return window._getGameCurrentChat ? window._getGameCurrentChat() : null; }

// ── Init — app.js se call karo ──────────────────────────
// currentUser ki jagah getCurrentUser getter pass karo
export function initGameSystem(db, getCurrentUser, getCurrentChat) {
  _db = db;
  _getCurrentUser = getCurrentUser;               // ✅ live getter
  window._getGameCurrentChat = getCurrentChat;    // ✅ live getter (already tha)
}
...
window.sendGameInvite = async function(gameType) {
  ...
  const gameData = {
    ...
    chatType,
    inviterId,
    inviterName: getU().name,
    inviterAvatar: getU().avatar || "?",
    createdAt: Date.now(),
  ...
  await set(ref(_db, `messages/${chatId}/${msgKey}`), {
    ...
    gameType,
    senderId:    inviterId,
    senderName:  getU().name,
    senderAvatar: getU().avatar || "?",
    timestamp:   ts,
    status:      "sent",
  });
  ...
  for (const mId of members) {
    await update(ref(_db, `user_chats/${mId}/${chatId}`), {
      lastMessage:     `🎮 ${getU().name} ne game invite bheja: ${gameName}`,
      lastMessageTime: ts,
    });
  }
...
window.acceptGameInvite = async function(gameId) {
  ...
  const updates = {
    status:    "active",
    acceptorId:    getU().uid,
    acceptorName:  getU().name,
    acceptorAvatar: getU().avatar || "?",
    startedAt:     Date.now(),
  };
  ...
  if (game.gameType === "ttt") {
    ...
    updates.playerO    = getU().uid;
    updates.winner     = null;
    updates.draw       = false;
  } else if (game.gameType === "rps") {
    updates.round      = 1;
    updates.maxRounds  = 3;
    updates.scores     = { [game.inviterId]: 0, [getU().uid]: 0 };
  ...
  modal.querySelector(".game-modal-inner").innerHTML = `
    <div class="game-header">
      ...
      <div class="game-players">
        <div class="gp ${isX ? 'me' : ''}">
          <span class="gp-avatar">${getU().avatar || "?"}</span>
...
window.playTTT = async function(gameId, cellIndex) {
  ...
  if (game.currentTurn !== getU().uid) return;
  if (game.board[cellIndex] !== null) return;
  if (game.status !== "active") return;

  const board    = [...game.board];
  const mySymbol = game.playerX === getU().uid ? "X" : "O";
  ...
  if (winner) {
    updates.winner  = getU().uid;
    updates.status  = "finished";
    // Notify in chat
    notifyGameResult(game.chatId, game.chatType, `🏆 ${getU().name} ne Tic-Tac-Toe jeeta!`);
  } else if (isDraw) {
    updates.draw   = true;
    updates.status = "finished";
  ...
  modal.querySelector(".game-modal-inner").innerHTML = `
    <div class="game-header">
      ...
      <div class="game-players">
        <div class="gp me">
          <span class="gp-avatar">${getU().avatar || "?"}</span>
          <span class="gp-name">${getU().name}</span>
    ...
    ` : (bothChose && !gameOver) ? `
      <div class="game-actions">
        ${game.inviterId === myId
          ...
          : `<div class="gi-waiting">⏳ Opponent next round start karega...</div>`
        }
      </div>
    ` : ""}
  `;
...
window.playRPS = async function(gameId, choice) {
  ...
  if (game.status !== "active") return;
  const choices = game.currentChoices || {};
  if (choices[getU().uid]) { showGameToast("Pehle se choose kar chuka hai! 😂"); return; }

  choices[getU().uid] = choice;
  ...
  if (choices[oppId]) {
    ...
    if (newRound > (game.maxRounds || 3)) {
      ...
      let resultMsg = "";
      if (myFinal > oppFinal)  resultMsg = `🏆 ${getU().name} ne Rock Paper Scissors jeeta ${myFinal}-${oppFinal}!`;
...
// ✅ FIX: Sirf inviter "Next Round" advance kare — double-trigger nahi hoga
window.nextRPSRound = async function(gameId) {
  const snap = await get(ref(_db, `games/${gameId}`));
  if (!snap.exists()) return;
  const game = snap.val();

  // Only inviter advances round (prevents both players writing simultaneously)
  if (game.inviterId !== getU().uid) {
    showGameToast("Opponent next round start karega! ⏳");
    return;
  }
  if (!game.roundReveal) return; // Already cleared
