...
window.sendMagicLink = async function() {
  ...
  try {
    ...
    // 🔄 Quota exceed? Button dikhao, redirect mat karo
    if (e.code === "auth/quota-exceeded") {
      showToast("Arey yaar, magic link ka quota khatam ho gaya 😅");
      localStorage.setItem("pendingLoginEmail", email);
      document.getElementById("quota-exceeded-options").classList.remove("hidden");
    } else {
...
window.switchToPhoneLogin = function() {
  ...
  document.getElementById("quota-exceeded-options").classList.add("hidden");
  document.getElementById("email-step").classList.add("hidden");
  document.getElementById("phone-otp-step").classList.remove("hidden");
  if (email) {
...
window.startVoiceRecording = async function(e) {
  if (e) e.preventDefault(); // touch events ke liye
...
function stopVoiceRecording() {
  ...
  if (voiceRecSeconds < 1) {
    showToast("Thoda zyada hold karo bhai! 😅");
...
window.searchGroupMember = window.searchGroupMember; window.addGroupMember = window.addGroupMember;
window.removeGroupMember = window.removeGroupMember; window.createGroup = window.createGroup;
window.toggleTimerPicker = window.toggleTimerPicker; window.setDestructTimer = window.setDestructTimer;
window.startVoiceRecording = window.startVoiceRecording; window.cancelVoiceRecording = window.cancelVoiceRecording;
