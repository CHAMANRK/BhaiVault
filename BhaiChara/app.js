// ╔══════════════════════════════════════════════════════╗
// ║  BhaiChara — app.js                                  ║
// ║  Firebase: Auth (Phone OTP) + Realtime Database      ║
// ╚══════════════════════════════════════════════════════╝
...
let chatListeners = {};  // active Firebase listeners
...
window.addEventListener("DOMContentLoaded", () => {
  populateEmojiGrid("emoji-grid", EMOJIS, (e) => { selectedEmoji = e; document.getElementById("avatar-initials").textContent = e; document.getElementById("emoji-picker").classList.add("hidden"); });
  ...
  onAuthStateChanged(auth, async (user) => {
    if (user) {
      ...
      if (snap.exists()) {
        ...
        currentUser = { uid: user.uid, phone: user.phoneNumber };
        showScreen("profile");
      }
    } else {
      currentUser = null;
...
window.sendOTP = async function() {
  ...
  if (phone.length !== 10 || !/^\d+$/.test(phone)) {
    showToast("Sahi phone number dalo (10 digits)"); return;
  }
  ...
  try {
    ...
    confirmResult = await signInWithPhoneNumber(auth, "+91" + phone, window.recaptchaVerifier);
    document.getElementById("phone-step").classList.add("hidden");
    document.getElementById("otp-step").classList.remove("hidden");
    showToast("OTP bhej diya! ✅");
  } catch (e) {
    showToast("OTP nahi gaya: " + (e.message || e.code));
    btn.disabled = false;
...
window.verifyOTP = async function() {
  ...
  if (code.length !== 6) { showToast("6 digit ka code dalo"); return; }
  ...
  try {
    ...
    showToast("Verify ho gaya! 🎉");
  } catch (e) {
    showToast("Galat OTP hai yaar");
    btn.disabled = false;
    btn.innerHTML = '<span>Verify Karo</span><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M20 6L9 17l-5-5"/></svg>';
  }
};
...
window.saveProfile = async function() {
  ...
  const profile = {
    name, status, avatar: selectedEmoji,
  ...
  const cleanPhone = currentUser.phone.replace("+91","").replace(/\D/g,"");
  await set(ref(db, `phone_index/${cleanPhone}`), currentUser.uid);
...
window.openChat = async function(chatId, type) {
  ...
  // Reset unread count
  const uid = currentUser.uid;
  update(ref(db, `user_chats/${uid}/${chatId}`), { unread: 0 });
...
function buildMessageEl(msg, type) {
  ...
  // Group: show sender name for incoming
...
window.sendMessage = async function() {
  ...
  // Update user_chats for all members
  ...
  for (const memberId of members) {
    ...
    if (!isMe) {
      ...
      // private: show sender's name/avatar; group: show group name/avatar
  ...
  // Mark as delivered after tiny delay
  setTimeout(() => {
...
window.startPrivateChat = async function(peerId) {
  ...
  // Ensure both user_chats entries exist
...
window.createGroup = async function() {
  ...
  // Add to user_chats for all members
...
window.verifyOTP = window.verifyOTP;
window.backToPhone = window.backToPhone;
