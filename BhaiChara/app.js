...
import { initGameSystem, buildGameMessageEl } from "./game.js";
import { initVideoCall } from "./videocall.js";
...
let isManualLogin = false;   // 🔒 OTP flow chal raha hai — onAuthStateChanged ignore karo
let otpAttempts   = 0;       // 🔢 Galat OTP count
...
window.addEventListener("DOMContentLoaded", async () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    // 🔒 OTP manual flow chal raha hai — is callback ko ignore karo
    // (signInAnonymously ka trigger hum khud handle karte hain)
    if (isManualLogin) return;

    if (user) {
      // Magic link login ya existing anon session
      ...
      if (snap.exists()) {
        ...
        // Magic link se aaya naya user — profile banana hai
        // Phone number hum profile step pe nahi maangenge (magic link flow)
        currentUser = { uid: realUid, email: user.email || "", _isMagicUser: true };
        showScreen("profile");
      }
    } else {
      currentUser = null;
...
//  SCREEN MANAGER
// ─────────────────────────────────────────────────────────
const AUTH_STEPS = ["phone-entry-step","otp-verify-step","magic-link-step","waiting-step","profile-step"];
...
function showScreen(name) {
  ...
  const choiceEl = document.getElementById("login-choice-screen");
  choiceEl.classList.add("hidden");
  authEl.classList.toggle("hidden", name === "app");
  appEl.classList.toggle("hidden",  name !== "app");

  if (name === "login" || name === "login-choice") {
    showStep("phone-entry-step");
    otpAttempts = 0;
    const wrap = document.getElementById("new-user-email-wrap");
    const fallback = document.getElementById("magic-link-fallback");
    if (wrap) wrap.classList.add("hidden");
    if (fallback) fallback.classList.add("hidden");
    const ph = document.getElementById("login-phone-input");
    const em = document.getElementById("login-email-input");
    if (ph) ph.value = "";
    if (em) em.value = "";
  }
  if (name === "profile") {
...
// Legacy aliases (game.js / other code might call these)
window.chooseMagicLink = function() { showMagicLinkStep(); };
window.choosePhoneOtp  = function() { showScreen("login"); };

function showApp() {
  showScreen("app");
  updateSidebarAvatar();
  setupPresence();
  initGameSystem(db, () => currentUser, () => currentChat); // 🎮 Game system init — getter se fresh currentUser milega
  initVideoCall(db, currentUser, () => currentChat);  // 📹 Video call init
  loadChatList();
  askNotificationPermission(); // 🔔 Notification permission maango
}

// ─────────────────────────────────────────────────────────
//  AUTH & PROFILE — Phone-first system
// ─────────────────────────────────────────────────────────

// ── Phone input ke saath live check ──
window.onPhoneInput = async function(input) {
  const phone = input.value.trim();
  const emailWrap = document.getElementById("new-user-email-wrap");
  if (phone.length === 10 && /^\d+$/.test(phone)) {
    try {
      const snap = await get(ref(db, `phone_index/${phone}`));
      if (snap.exists()) {
        // Returning user — email nahi maangna
        emailWrap.classList.add("hidden");
        document.getElementById("phone-entry-hint").textContent = "Wapas aa gaye! 🔥 OTP se login karo.";
      } else {
        // Naya user — email maango
        emailWrap.classList.remove("hidden");
        document.getElementById("phone-entry-hint").textContent = "Pehli baar aa rahe ho! 👋";
      }
    } catch(e) {
      // Silently fail — form pe asar nahi
    }
  } else {
    emailWrap.classList.add("hidden");
    document.getElementById("phone-entry-hint").textContent = "OTP aayega — seedha aur fast! 📱";
  }
};
...
window.sendOtp = async function() {
  ...
  if (!phone || phone.length !== 10 || !/^\d+$/.test(phone)) {
    showToast("10 digit ka sahi number dalo bhai!"); return;
  }
  ...
  try {
    ...
    if (!snap.exists()) {
      ...
      if (!email || !email.includes("@")) {
        showToast("Naye account ke liye email bhi dalo bhai!"); return;
      }
      // Email already kisi aur ke paas to nahi?
      const emailSnap = await get(ref(db, `email_index/${email.replace(/\./g, "_")}`));
      if (emailSnap.exists()) {
        showToast("Ye email pehle se registered hai 🤔 Apna number use karo."); return;
      }
      ...
      localStorage.setItem("otpUid", existingUid);
    }
  } catch(e) {
    showToast("DB error: " + e.message); return;
  }
  ...
  try {
    ...
    if (!apiRes.ok) {
      showToast("SMS error: " + (apiData.error || "Dobara try karo"));
      btn.disabled = false;
      ...
      // SMS fail — magic link dikhao
      document.getElementById("magic-link-fallback").classList.remove("hidden");
      return;
    }
    // OTP sent ✅
    otpAttempts = 0;
    ...
    document.getElementById("otp-attempts-hint").style.display = "none";
    document.getElementById("otp-magic-fallback").classList.add("hidden");
    showStep("otp-verify-step");
    showToast("OTP bhej diya! 📱 SMS check karo");
  } catch(err) {
  ...
  // NOTE: Success case mein btn reset nahi karte — user OTP screen pe chala gaya hai
};
...
window.verifyOtp = async function() {
  ...
  const isNewUser  = localStorage.getItem("otpIsNewUser") === "true";
  const savedUid   = localStorage.getItem("otpUid");

  if (!phone) { showToast("Phone number nahi mila, wapas jao"); return; }
  if (!enteredOtp || enteredOtp.length < 4) {
    showToast("OTP dalo bhai!"); return;
  }
  ...
  try {
    ...
    if (!apiRes.ok) {
      ...
      btn.disabled = false;
      btn.innerHTML = '<span>Verify Karo ✓</span>';

      const remaining = 3 - otpAttempts;
      const hintEl = document.getElementById("otp-attempts-hint");
      if (otpAttempts >= 3) {
        hintEl.textContent = "3 baar galat OTP — Magic link try karo!";
        hintEl.style.display = "block";
        document.getElementById("otp-magic-fallback").classList.remove("hidden");
        showToast("Galat OTP 😅 Magic link try karo?");
      } else {
        hintEl.textContent = `Galat OTP — ${remaining} chance bache hain`;
        hintEl.style.display = "block";
        showToast(apiData.error || "Galat OTP bhai! 🙅");
      }
      return;
    }

    // ✅ OTP sahi — ab Firebase session banao
    isManualLogin = true; // onAuthStateChanged mute karo
    try {
      const anonCred = await signInAnonymously(auth);
      const anonUid  = anonCred.user.uid;

      if (!isNewUser && savedUid) {
        // ── PURANA USER: seedha login ──
        const userSnap = await get(ref(db, `users/${savedUid}`));
        if (!userSnap.exists()) {
          showToast("Account data nahi mila 😕 Dobara try karo");
          await signOut(auth);
          isManualLogin = false;
          btn.disabled = false;
          btn.innerHTML = '<span>Verify Karo ✓</span>';
        ...
        currentUser = { uid: savedUid, ...userSnap.val() };
        await set(ref(db, `anon_map/${anonUid}`), savedUid);
        localStorage.removeItem("otpPhone");
        localStorage.removeItem("otpUid");
        localStorage.removeItem("otpIsNewUser");
        isManualLogin = false;
        showToast("Wapas aa gaya bhai! 🔥");
        showApp();
      } else {
        // ── NAYA USER: profile banwao ──
        const savedEmail = localStorage.getItem("otpEmail") || "";
        currentUser = {
          uid: phone,          // phone = primary uid
          email: savedEmail,
          phone: "+91" + phone,
          _anonUid: anonUid
        };
        await set(ref(db, `anon_map/${anonUid}`), phone);
        localStorage.removeItem("otpPhone");
        localStorage.removeItem("otpEmail");
        localStorage.removeItem("otpIsNewUser");
        isManualLogin = false;
        showToast("OTP sahi hai! 🎉 Profile banao");
        showScreen("profile");
      }
    } catch(firebaseErr) {
      isManualLogin = false;
...
window.backToPhoneEntry = function() {
  ...
  document.getElementById("otp-attempts-hint").style.display = "none";
  document.getElementById("otp-magic-fallback").classList.add("hidden");
  // Button reset karo taaki dobara use ho sake
  const btn = document.getElementById("send-otp-btn");
  if (btn) {
    btn.disabled = false;
    btn.innerHTML = '<span>OTP Bhejo</span><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M5 12h14M12 5l7 7-7 7"/></svg>';
...
window.showMagicLinkStep = function() {
  // Pre-fill email if we have it
  const saved = localStorage.getItem("otpEmail") || "";
  const magicEl = document.getElementById("magic-email-input");
  if (magicEl && saved) magicEl.value = saved;
  showStep("magic-link-step");
};

window.backToMagicLinkStep = function() {
  showStep("magic-link-step");
...
window.saveProfile = async function() {
  ...
  // uid = phone number (set during OTP verify) ya magic-link Firebase uid
  const uid   = currentUser.uid;
  const phone = currentUser.phone || "";   // "+91XXXXXXXXXX" ya ""
  const email = currentUser.email || auth.currentUser?.email || "";
  const rawPhone = phone.replace("+91", ""); // "XXXXXXXXXX"

  const profile = { name, status, avatar: selectedEmoji, phone, email, createdAt: Date.now() };

  try {
    await set(ref(db, `users/${uid}`), profile);
    if (rawPhone) {
      await set(ref(db, `phone_index/${rawPhone}`), uid);
    }
    if (email) {
      await set(ref(db, `email_index/${email.replace(/\./g, "_")}`), uid);
    }
...
window.triggerBhaiNotification = function(title, body) {
  ...
  // 🔔 Push Notification — permission granted hona chahiye
  ...
  try {
    ...
    const notif = new Notification(`${randEmoji} ${title}`, {
      body: body || "Naya message aaya!",
      icon: "logo-192.png",
      badge: "logo-192.png",
      tag: "bhaichara-msg",          // Same tag = purani replace hogi, spam nahi
      renotify: true,                 // Same tag hone par bhi sound bajao
      silent: false
    });
    // Notification click karne par app focus ho
...
function loadChatList() {
  ...
  // 🔔 FIX: Pehla snapshot seedha previousChatsState mein store karo — notification mat fire karo
  ...
  onValue(chatListRef, async (snap) => {
    ...
    for (const [chatId, meta] of Object.entries(chats)) {
      ...
      if (!isFirstSnapshot && prev && meta.lastMessageTime > (prev.lastMessageTime || 0) && !isChatOpen) {
        triggerBhaiNotification(meta.name || "Bhai", meta.lastMessage || "Naya message aaya!");
      }
    ...
    isFirstSnapshot = false; // 🔔 Pehla snapshot process ho gaya — ab notifications fire honge
  });
}
...
function renderChatList(items) {
  ...
  container.innerHTML = items.map(item => {
    ...
    return `
      <div class="chat-item${isActive?" active":""}" data-chatid="${item.chatId}" data-chattype="${item.type}" onclick="openChat('${item.chatId}','${item.type}')">
  ...
  container.querySelectorAll(".chat-item").forEach(el => {
    ...
    const name     = el.querySelector(".ci-name")?.textContent || "Chat";
    const avatar   = el.querySelector(".ci-avatar")?.textContent?.trim()?.[0] || "?";

    function showCtxMenu(e) {
      if (e && e.cancelable) e.preventDefault();
      _ctxChatId   = chatId;
      _ctxChatType = chatType;
      document.getElementById("ctx-name").textContent   = name;
      document.getElementById("ctx-avatar").textContent = avatar;
      document.getElementById("chat-context-menu").classList.remove("hidden");

      // Outside tap se band ho jaye
      setTimeout(() => {
        function outsideClose(ev) {
          const menu = document.getElementById("chat-context-menu");
          const inner = menu?.querySelector(".chat-context-menu-inner");
    ...
    el.addEventListener("touchstart", () => {
      _pressTimer = setTimeout(showCtxMenu, 600);
    }, { passive: true });
    el.addEventListener("touchend",   () => clearTimeout(_pressTimer));
    el.addEventListener("touchmove",  () => clearTimeout(_pressTimer));

    // Desktop — right click
    el.addEventListener("contextmenu", showCtxMenu);
  });
}
...
window.openChat = async function(chatId, type) {
  ...
  // 📹 Video call button — sirf private chat mein dikhao
  const vcBtn = document.getElementById("vc-call-btn");
  if (vcBtn) vcBtn.classList.toggle("hidden", type !== "private");
...
function buildMessageEl(msg, type) {
  ...
  // 🔊 Receive sound — sirf incoming naye messages ke liye
  if (!isOut && msg.timestamp && (Date.now() - msg.timestamp) < 4000) {
    try { if (window.SFX) window.SFX.receive(); } catch(e) {}
  }
  ...
  // 🎮 GAME MESSAGES
  if (msg.type === "game_invite" || msg.type === "game_result") {
    return buildGameMessageEl(msg, currentUser);
  }
  ...
  const bubble = div.querySelector(`.${bubbleClass.split(" ")[0]}`);

  function showMsgDeleteMenu(e) {
    if (e && e.cancelable) e.preventDefault();
    // Remove any existing msg context menu
    ...
    menu.innerHTML = `
      <div class="msg-context-overlay" id="msg-context-overlay"></div>
      <div class="msg-context-inner ${isOut ? 'align-right' : 'align-left'}">
        <button class="msg-context-delete-btn" id="msg-ctx-delete-btn">
    ...
    document.getElementById("msg-context-overlay").addEventListener("click", () => menu.remove());

    // Delete action
    document.getElementById("msg-ctx-delete-btn").addEventListener("click", async () => {
      menu.remove();
      if (!currentChat) return;
      // Find the message key by matching timestamp + senderId
      const msgsSnap = await get(ref(db, `messages/${currentChat.id}`));
      const msgs = msgsSnap.val() || {};
      const keyToDelete = Object.entries(msgs).find(([, m]) => m.timestamp === msg.timestamp && m.senderId === msg.senderId)?.[0];
      if (keyToDelete) {
        await set(ref(db, `messages/${currentChat.id}/${keyToDelete}`), null);
  ...
  div.addEventListener("touchstart", () => {
    _msgPressTimer = setTimeout(showMsgDeleteMenu, 600);
  }, { passive: true });
  div.addEventListener("touchend",  () => clearTimeout(_msgPressTimer));
  div.addEventListener("touchmove", () => clearTimeout(_msgPressTimer));
  // Desktop — right click
  div.addEventListener("contextmenu", showMsgDeleteMenu);
...
// ── DELETE CHAT (sirf apne liye remove karo) ──────────────
async function deleteChatById(chatId, type) {
  if (!currentUser) return;
  try {
    // Sirf apni chat list se hatao — dusre ke liye rehne do
    await set(ref(db, `user_chats/${currentUser.uid}/${chatId}`), null);
    // Agar current chat yahi thi to welcome screen pe wapas lo
    ...
    showToast("Chat delete ho gayi! 💀");
  } catch (e) { showToast("Error: " + e.message); }
}

window.deleteChat = async function() {
  if (!currentChat) return;
  if (!window.confirm("Ye chat teri list se hata di jayegi. Pakka?")) return;
  closeModal('chat-quick-settings-modal');
  await deleteChatById(currentChat.id, currentChat.type);
};

// ── LONG PRESS CONTEXT MENU ───────────────────────────────
let _ctxChatId = null, _ctxChatType = null;

window.closeContextMenu = function() {
  document.getElementById("chat-context-menu").classList.add("hidden");
  _ctxChatId = null; _ctxChatType = null;
};

window.deleteChatFromContext = async function() {
  const chatId = _ctxChatId, type = _ctxChatType;
  closeContextMenu();
  if (!chatId) return;
  if (!window.confirm("Ye chat delete karni hai? Teri list se hat jayegi.")) return;
  await deleteChatById(chatId, type);
};
...
// Global exports 
// Auth exports (functions are already on window via window.xxx = ...)
window.sendOtp = window.sendOtp; window.verifyOtp = window.verifyOtp;
window.sendMagicLink = window.sendMagicLink; window.saveProfile = window.saveProfile; window.pickAvatar = window.pickAvatar;
window.backToPhoneEntry = window.backToPhoneEntry; window.showMagicLinkStep = window.showMagicLinkStep; window.backToMagicLinkStep = window.backToMagicLinkStep;
window.onPhoneInput = window.onPhoneInput;
...
window.openChatQuickSettings = window.openChatQuickSettings; window.clearChat = window.clearChat; window.deleteChat = window.deleteChat; window.deleteChatFromContext = window.deleteChatFromContext; window.closeContextMenu = window.closeContextMenu; window.askNotificationPermission = window.askNotificationPermission;
...
// 🎮 Game exports (game.js mein window. se directly attach hote hain)
window.openGameLobby = window.openGameLobby; window.closeGameLobby = window.closeGameLobby;
window.sendGameInvite = window.sendGameInvite; window.acceptGameInvite = window.acceptGameInvite; window.declineGameInvite = window.declineGameInvite;
window.openGameBoard = window.openGameBoard; window.closeGameBoard = window.closeGameBoard;
window.playTTT = window.playTTT; window.playRPS = window.playRPS; window.nextRPSRound = window.nextRPSRound;
// 📹 Video call exports (videocall.js mein window. se attach hote hain)
window.startVideoCall = window.startVideoCall; window.acceptVideoCall = window.acceptVideoCall;
window.declineVideoCall = window.declineVideoCall; window.endVideoCall = window.endVideoCall;
window.toggleMute = window.toggleMute; window.toggleCamera = window.toggleCamera; window.switchCamera = window.switchCamera;
