...
window.addEventListener('visibilitychange', () => {
  if (document.hidden && hasAnyProviderKey() && currentSession.length >= 2) autosaveSession();
});

window.addEventListener('beforeunload', () => {
  if (hasAnyProviderKey() && currentSession.length >= 2) autosaveSession(); // best-effort, fire-and-forget — page may close before it lands
});
...
(function bindEvents() {
  ...
  document.querySelectorAll('.icon-btn').forEach(btn => {
    ...
    if (title === 'Memory') btn.addEventListener('click', openMemModal);
    if (title === 'Settings') btn.addEventListener('click', openSettings);
    if (title === 'New Chat') btn.addEventListener('click', newChat);
  });
