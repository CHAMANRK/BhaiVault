...
async function performWebSearch(query) {
  ...
  return withTimeout(async (signal) => {
    ...
    // ── DEBUG: temporary — Vercel function logs mein exact failure point dikhane ke liye.
    // Root-cause confirm hote hi ye poora block hata dena.
    const bodyPreview = await r.clone().text().catch(() => '<could not read body>');
    console.log(
      `[web_search DEBUG] query="${query}" status=${r.status} ok=${r.ok} bodyLen=${bodyPreview.length}\n` +
      `[web_search DEBUG] bodyPreview(first 500 chars):\n${bodyPreview.slice(0, 500)}`
    );
    // ── END DEBUG
    ...
    // ── DEBUG
    console.log(`[web_search DEBUG] parsed ${results.length} result(s) for "${query}"`);
    // ── END DEBUG
