...
export async function scheduleShiftAlerts() {
  ...
  try {
    const pSnap = await get(ref(_database, 'settings/workPressureMode'));
...
export function startRepeatReminder(shiftKey, shift) {
  ...
  _repeatTimers[shiftKey] = setInterval(async () => {
    ...
    try {
      const snap = await get(ref(_database, `attendance/${_currentUser.uid}/${today}/${shiftKey}`));
      const data = snap.val();
...
function listenWorkPressureMode() {
  ...
  onValue(ref(_database, 'settings/workPressureMode'), (snap) => {
