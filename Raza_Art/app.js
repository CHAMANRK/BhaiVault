...
export async function checkIn(userId, shiftKey, locationData = null) {
  ...
  // Notify owner
  try {
    const userSnap = await get(ref(db, `users/${userId}/name`));
    const staffName = userSnap.val() || 'Staff';
    const shiftLabel = SHIFTS[shiftKey]?.name || shiftKey;
    const timeStr  = formatTime(now);
    const statusLabel = late ? '⚠️ Late' : '✅ On Time';

    await push(ref(db, 'owner_notifications'), {
      type      : 'checkin',
      staffUid  : userId,
      staffName,
      shiftKey,
      ...
      late,
      statusLabel,
      message   : `${staffName} ne ${shiftLabel} mein check-in kiya — ${timeStr} (${statusLabel})`,
      createdAt : now,
      read      : false
    });
  } catch (e) {
    console.warn('[App] Owner notify (checkIn) failed:', e);
  }
...
export async function checkOut(userId, shiftKey) {
  ...
  try {
    ...
    await push(ref(db, 'owner_notifications'), {
      ...
      timeStr,
      durStr,
      message   : `${staffName} ne ${shiftLabel} mein check-out kiya — ${timeStr} (${durStr} kaam kiya)`,
      createdAt : now,
      read      : false
    });
  } catch (e) {
    console.warn('[App] Owner notify (checkOut) failed:', e);
  }
    ...
    try {
      ...
      if (activeShiftKey) {
        ...
        if (locationData) {
          ...
          if (!proximity.ok) {
            // Store location is set but staff is too far
            showToast(`📍 Too far from store (${proximity.distance}m away). Come closer to check in.`, 'error');
            return; // don't check in
          }
        }
        // If locationData is null (GPS failed) and store location IS set, still check — store check happens server-side
        // If no store location set at all, proximity.ok = true anyway (getStoreLocation returns null)

        const shift = pendingShiftKey; // check-in for this specific shift
        if (!navigator.onLine) {
          const rec = {
