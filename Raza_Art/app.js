...
if (offlineBadge) {
  // navigator.onLine is unreliable on Android with Bluetooth
  // Use actual network test instead
  ...
  // Check on load only if browser says offline
...
export function calcDailySummary(todayAttendance, allStaff) {
  ...
  // If allStaff not loaded yet, use attendance count as minimum
...
async function initStaffDashboard(user, profile) {
  ...
  try {
    todayRecord = await getTodayAttendance(user.uid);
  } catch(e) {
