/* ============================================================
   app.js  —  WorkTrack PWA  —  Core Application Logic
...
// ════════════════════════════════════════════
export const SHIFTS = {
  shift1: { name: 'Shift 1', start: '08:00', end: '16:00', label: '8:00 AM – 4:00 PM' },
  shift2: { name: 'Shift 2', start: '16:00', end: '00:00', label: '4:00 PM – 12:00 AM' }
};
...
export function detectCurrentShift() {
  ...
  const mins = h * 60 + m;
  if (mins >= 8 * 60 && mins < 16 * 60) return 'shift1';
  if (mins >= 16 * 60) return 'shift2';
  return null;
}
...
export async function checkIn(userId, shiftKey, locationData = null) {
  ...
  await set(ref(db, `attendance/${todayKey()}/${userId}`), record);
  return record;
}
...
export async function checkOut(userId) {
  ...
  await update(ref(db, `attendance/${todayKey()}/${userId}`), updates);
...
export async function flushPendingAttendance() {
  ...
  for (const item of pending) {
    try {
      await set(ref(db, `attendance/${item.date}/${item.userId}`), item.record);
    } catch (e) {
...
export function calcStats(attendanceMap) {
  const records = Object.values(attendanceMap).filter(Boolean);
...
export function calcDailySummary(todayAttendance, allStaff) {
  ...
  const attended   = Object.values(todayAttendance).filter(r => r?.checkIn).length;
  const lateCount  = Object.values(todayAttendance).filter(r => r?.late).length;
  const absent     = staffCount - attended;
...
function renderCheckinButton(record, userId, profile) {
  ...
  const update_ = (rec) => {
    if (rec?.checkIn && !rec?.checkOut) {
      ...
      if (statusEl) statusEl.innerHTML = statusBadge(rec.status);
      if (timeEl)   timeEl.textContent = `In: ${formatTime(rec.checkIn)}`;
      ...
      if (statusEl) statusEl.innerHTML = statusBadge(rec.status);
      if (timeEl)   timeEl.textContent = `${formatTime(rec.checkIn)} – ${formatTime(rec.checkOut)} · ${formatDuration(rec.duration)}`;
    } else {
      // Not checked in yet
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      const current = await getTodayAttendance(userId);

      if (!current?.checkIn) {
        // Perform check-in with optional geo
        let locationData = null;
        try {
          locationData = await getCurrentPosition();
        } catch {
          // Location not required, continue
        }
        const shift = detectCurrentShift() || 'shift1';
        if (!navigator.onLine) {
          const rec = {
          ...
          update_(rec);
          showToast(rec.late ? '⚠️ Checked in late!' : '✅ Checked in successfully!', rec.late ? 'warning' : 'success');
        }
      } else if (!current.checkOut) {
        // Perform check-out
        const rec = await checkOut(userId);
        update_(rec);
        showToast(`🏁 Checked out! Duration: ${formatDuration(rec.duration)}`, 'success');
      }
    } catch (err) {
...
async function initOwnerDashboard(user, profile) {
  ...
  window._allStaff = allStaff; // cache for re-renders
  ...
  const unsubLeaves = listenPendingLeaves((leaves) => {
    ...
    if (badge) badge.textContent = leaves.length || '';
  });

  // Announcements
  listenAnnouncements((items) => renderAnnouncements(items));

  // Work Pressure Toggle
  const pressureToggle = document.getElementById('workPressureToggle');
  const pressureInput  = pressureToggle?.querySelector('input');
  if (pressureInput) {
    listenWorkPressure((wp) => {
      pressureInput.checked = !!wp.active;
      if (wp.active) activatePressureBanner(wp.message);
      else deactivatePressureBanner();
    });

    pressureInput.addEventListener('change', async () => {
      const active = pressureInput.checked;
      const msg    = active ? (document.getElementById('pressureMsg')?.value || 'High workload mode active') : '';
      await setWorkPressure(active, msg);
      showToast(active ? '🔴 Work pressure mode ON' : '🟢 Work pressure mode OFF', active ? 'warning' : 'success');
    });
  }
  ...
  if (announceForm) {
    announceForm.addEventListener('submit', async (e) => {
      ...
      if (!msg) return;
      await postAnnouncement(user.uid, msg);
      announceForm.reset();
...
function renderAttendanceList(todayAtt, allStaff) {
  ...
  container.innerHTML = allStaff.map(staff => {
    ...
    const checkIn  = rec?.checkIn  ? formatTime(rec.checkIn)  : '--:--';
    const checkOut = rec?.checkOut ? formatTime(rec.checkOut) : '--:--';
